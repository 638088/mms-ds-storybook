...
`,o={name:`What's New`,render:()=>t`
  ...
  render: () => html\`
    <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; max-width: \${PAGE_MAX}; margin: 0 auto; padding: 2rem; line-height: 1.6; color: inherit;">
      ...
      \${entry('v0.24.0 — Pre-release — 2026-08-28', true, [{
    heading: 'Repository & CI',
    items: ['The platform moved from a personal GitHub account to the Maximus enterprise org maximus-atx-ce. A git push --mirror copy, not a transfer — the personal repos stay live and untouched, and issue/PR discussion threads remain readable there. Only git history came across, which the file-based tracker makes a non-issue (DS-169)', 'GitHub Actions is disabled org-wide and cannot be enabled, so .github/workflows/ci.yml was deleted and replaced by npm run ci — a portable 16-step gate that runs in a throwaway worktree with a fresh npm ci, forcing CI=1 so it reproduces the hosted pipeline rather than a materially different local one (DS-169)', 'Merge enforcement now exists for the first time. Branch protection was never available on a private personal repo, so nothing had ever blocked a merge; the gate now posts an mms/ci commit status and an org ruleset requires it on main. Because status binds to a SHA, a new push resets the gate to red automatically (DS-169)', '/pr-merge Gate 3 was rewritten around this — it gates the PR head SHA and posts the status rather than polling gh pr checks for a workflow that no longer runs (DS-169)']
  }, {
    heading: 'Accessibility',
    items: ['The a11y coverage record is now traceable and committed rather than gitignored and per-machine. Each result carries the commit SHA plus a hash of the inputs that component depends on — its source, CSS and colour-type files, the 13-file shared surface, and the pinned axe-core / @axe-core/playwright / @playwright/test versions, since the rule engine decides what counts as a violation. A result whose inputs changed is invalid, not stale-but-acceptable. Before this, a scoped run reported all 26 components green with 25 of those greens historical and of unknown age (DS-169)', 'New /build-component Phase 8 names the refinement loop and forbids test invocations inside it. The loop was real work with no documented shape, so agents fell back on "run the tests" after every edit — five recurrences. A11y runs once at the end of the build, then not again until J invokes /test-component (DS-169)']
  }, {
    heading: 'Platform Tooling',
    items: ['The CI gate ledger is one file per commit under test-results/ci-runs/<sha>.json, written once and never edited. A single shared array put every record on the same line, so once main and a feature branch both held one, every merge conflicted there — and rewriting a tracked file made git checkout main refuse mid-/pr-merge (DS-169)', 'The ledger is now staged by both flows that push directly to main, and .husky/pre-commit refuses a main commit that leaves a record unstaged. A record is written after its own commit, so it can only ever be carried by the next one — that lag is inherent, and nothing reads the ledger, so a dropped record was lost silently. This is how the first one went missing (DS-169)', 'Fixed a push sequence in /pr-merge and /update-docs that could not work: both asserted the Commit Status API accepts a status for a SHA the remote has never seen. It answers 422, and the push is separately rejected for the missing status. Broken by first publishing the commit under refs/ci/*, which the ruleset does not govern (DS-169)']
  }])}

      \${entry('v0.23.0 — Pre-release — 2026-08-21', false, [{
